...
/**
 * For a device to switch connection between SAP Mobile Platform servers, this API is invoked before registering a user with new server. This ensures that the server public keys are removed from the SAP Mobile Platform client SDK which enables connectivity to new SAP Mobile Platform server.
 * <pre>
 *  Usage
 * <code>
...
/**
 * Set the server verification key
 * <pre>
 *  Usage
 * <code>
 *  ODPClientConnection* clientConnection = [ODPClientConnection getInstance:@"ApplicationId"];
 *  NSError* error = nil;
 *  if (![clientConnection setServerVerificationKey:keyValue withError:&error])
 *  {
 ...
 * </pre>
 */
-(BOOL)setServerVerificationKey:(NSString *)serverKey withError:(NSError**)error;
...
/**
 * Registers for push notification using device token as a parameter. After the application is registered for APNS push notification, pass the device token received to SAP Mobile Platform server. The device token must be sent to server to send the notification through APNS.
...
#pragma mark ResetClientConnection Api

/**
 * For a device to switch connection between SUP servers present on different networks, this API is invoked before registering a user with new server. This ensures that the client state is reset before registering with different server on different or same network.
 * <pre>
 *  Usage
 * <code>
 *  [ODPClientConnection resetClientConnection];
 * </code>
 * </pre>
 */
