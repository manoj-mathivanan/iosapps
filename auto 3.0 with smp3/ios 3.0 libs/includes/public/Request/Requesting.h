...
/**
 * Returns the maximum number of Request threads which can be executed in parallel.
 */
 ...
 */
- (void)setEtag:(NSString *)etag withMatchType:(EetagMatchType) matchType;
...
/**
 * Gets the response of a request. It is uncompressed if required.
 * @pram None
...
/**
 * Runs request asynchronously, in the background.
...
/**
 * Clients can chose to register their custom selectors to be invoked after request completes executing.
