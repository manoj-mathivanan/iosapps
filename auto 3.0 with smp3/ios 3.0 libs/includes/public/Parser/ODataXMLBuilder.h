...
@interface ODataEntryXML : NSObject {
	NSString*	m_Xml;
	NSString*	m_Method;
}
 ...
 */
@interface ODataEntryJSON : NSObject {
	NSString*	m_Json;
	NSString*	m_Method;
}
...
typedef ODataEntryXML ODataFeedXML;
 ...
 * <code>
 * ODataEntryXML* entryAsXmlAndHttpMethod = buildODataEntryXML(entry, ENTRY_OPERATION_UPDATE_FULL, serviceDocument, NO);
 * </code>
 ...
 * @param entry The entry from which the function creates its xml representation (@see ODataEntry)
 ...
 * </pre>
 */
ODataEntryXML* buildODataEntryXML(const ODataEntry* const entry, const enum TEN_ENTRY_OPERATIONS operation,
 ...
 */
ODataFeedXML* buildODataFeedXML(NSArray* const entries, const enum TEN_ENTRY_OPERATIONS operation,
									  const ODataServiceDocument* const serviceDocument, 
									  const BOOL serializeInlinedEntries);
/**
 ...
 * @example Creating a json from an entry that can be sent to the server to create or update the data in the backend defined by the key values.
 * <code>
 *  ODataEntryJSON* entryAsJsonAndHttpMethod = buildODataEntryJSON(entry, ENTRY_OPERATION_UPDATE_FULL);
 *  NSString *jsonBody = entryAsJsonAndHttpMethod.json;
 * </code>
 */
ODataEntryJSON* buildODataEntryJSON(const ODataEntry* const entry, const enum TEN_ENTRY_OPERATIONS operation);
